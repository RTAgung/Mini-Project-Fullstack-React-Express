...
const loadSeeders = async (
    pathName: string
): Promise<{ file: string[]; seeder: any }[]> => {
    ...
    const seederImports = files.map(
        (file) => import(pathToFileURL(path.join(pathName, file)).href)
    );
...
const collectSeeders = async () => {
    let seeders = await Promise.all([
        ...(await loadSeeders(__dirname + "./cores")),
    ]);
    let appSeeders = await Promise.all(
        ["./cores"].map((appPath: string) =>
            loadSeeders(path.join(__dirname, appPath))
        )
    );
